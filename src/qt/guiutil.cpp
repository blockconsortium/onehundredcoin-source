...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("onehundredcoin"))
        return false;
...
{
    // Convert onehundredcoin:// to onehundredcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("onehundredcoin://"))
    {
        uri.replace(0, 12, "onehundredcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OneHundredCoin.lnk";
}
...
{
    return GetAutostartDir() / "onehundredcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=OneHundredCoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OneHundredCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  onehundredcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("OneHundredCoin-Qt"));
